...
public class OssUtils {
  ...
  static {
    try (InputStream inStream = OssUtils.class.getResourceAsStream("/oss.properties")) {
      Properties p = new Properties();
      p.load(inStream);
      endpoint = p.getProperty("endpoint");
      accessKeyId = p.getProperty("AccessKeyID");
      accessKeySecret = p.getProperty("AccessKeySecret");
      bucketName = p.getProperty("bucket");
      cname = p.getProperty("cname");
      pathPattern = p.getProperty("path.pattern");
   ...
   * @author yingjianhua
   */
  public static String upload(Class<?> beanClass, String filename, File file) {
    String fileKey =
  ...
  public static String upload(Class<?> beanClass, String url) throws IOException {
    File file = FileDownload.toTemp(url);
    String filename = MD5Utils.getMD5(file) + "." + FileDownload.getSuffix(url);
    return upload(beanClass, filename, file);
  }

  // =====START
  public static String upload(Class<?> beanClass, String filename, InputStream file) {
    String fileKey =
        pathPattern.replaceAll("%m", moduleName(beanClass)).replaceAll("%b", beanName(beanClass))
            + filename;
    logger.info("upload:" + fileKey);
  ...
  private static String upload(String fileKey, InputStream file) {
  ...
  // 取bean的模块名
  public static String moduleName(Class<?> beanClass) {
  ...
  public static String getDomain() {
    return cname == null ? ("http://" + bucketName + "." + endpoint) : ("http://" + cname);
  }
}
